...
#在这儿定义了图像特征的获取，对于snli的数据而言，其有对应的图像特征，对于vgenome数据而言，其特征定义为随机高斯噪声。
def get_snli_mnli_image_feature(sequences,image_feature):
    tensor_list = []
    for sequence in zip(sequences):
        #print(sequence)
        sequence =  "".join(sequence)
        if "jpg" in sequence:#表示是snli的数据
            #print("snli jpg")
            image_name = sequence.split("#")[0]
            #print(image_name)
            tensor_list.append(image_feature[image_name])
            #print(image_feature[image_name])
        else:
            #print("mnli jpg")
            vec = np.zeros(25088) #7*7*512
            tensor_list.append(vec)
