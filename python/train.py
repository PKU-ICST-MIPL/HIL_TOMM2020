"""
Training script to train a model on SNLI data, you can omit the MNLI part.
...
if config.debug_model:
    ...
    image_feature = load_image_feature(FIXED_PARAMETERS["image_feature"])     
...
class modelClassifier:
    ...
    def get_minibatch(self, dataset,start_index, end_index, training=False):
        ...
        image_feature = get_snli_mnli_image_feature([dataset[i]['pairID'] for i in indices],self.IMAGE_FEATURE)        
        ...
        return premise_vectors, hypothesis_vectors, labels, genres, premise_pos_vectors, \
                ...
                premise_exact_match, hypothesis_exact_match,image_feature
                


    def train(self, train_mnli, train_snli, dev_mat, dev_mismat, dev_snli,test_snli,image_feature):
        ...
        if os.path.isfile(ckpt_file + ".meta"):
            if os.path.isfile(ckpt_file + "_best.meta"):
                ...
                best_test_snli, test_cost_snli, _ = evaluate_classifier(self.classify, test_snli, self.batch_size)
                ...
                if self.alpha != 0.:
                    ...
                    logger.Log("Restored best matched-dev acc: %f\n Restored best mismatched-dev acc: %f\n Restored best SNLI-dev acc: %f\n Restored best SNLI-test acc: %f\n Restored best MulitNLI train acc: %f\n Restored best SNLI train acc: %f" %(dev_acc_mat, best_dev_mismat, best_dev_snli, best_test_snli,  self.best_mtrain_acc,  self.best_strain_acc))
                else:
                ...
                if config.training_completely_on_snli:
                    self.best_dev_mat = best_test_snli
            else:
        ...
        while True:
            ...
            for i in range(total_batch):
                ...
                premise_exact_match, hypothesis_exact_match,minibatch_image_feature  = self.get_minibatch(
                ...
                feed_dict = {self.model.premise_x: minibatch_premise_vectors,
                                ...
                                self.model.image_feature:minibatch_image_feature,
                                self.model.batch_len:batch_len}
                ...
                if self.step % self.eval_step == 0:
                    ...
                    if config.training_completely_on_snli:
                            ...
                            test_acc_snli,test_cost_snli, _ = evaluate_classifier(self.classify, test_snli, self.batch_size)
                        ...
                        dev_acc_mismat, dev_cost_mismat, dev_acc_snli, dev_cost_snli, test_scc_snli, test_cost_snli = 0,0,0,0,0,0

                    if self.dont_print_unnecessary_info and config.training_completely_on_snli:
                        mtrain_acc, mtrain_cost, = 0, 0
                    else:
                        mtrain_acc, mtrain_cost, _ = evaluate_classifier(self.classify, train_mnli, self.batch_size)
                    ...
                    if self.alpha != 0.:
                        ...
                        logger.Log("Step: %i\t Dev-matched acc: %f\t Dev-mismatched acc: %f\t Dev-SNLI acc: %f\t test-SNLI acc: %f\t MultiNLI train acc: %f\t SNLI train acc: %f" %(self.step, dev_acc_mat, dev_acc_mismat, dev_acc_snli, test_acc_snli, mtrain_acc, strain_acc))
                        logger.Log("Step: %i\t Dev-matched cost: %f\t Dev-mismatched cost: %f\t Dev-SNLI cost: %f\t test-SNLI cost: %f\t MultiNLI train cost: %f\t SNLI train cost: %f" %(self.step, dev_cost_mat, dev_cost_mismat, dev_cost_snli, test_cost_snli, mtrain_cost, strain_cost))
                    else:
                        logger.Log("Step: %i\t Dev-matched acc: %f\t Dev-mismatched acc: %f\t Dev-SNLI acc: %f\t test-SNLI acc: %f\t MultiNLI train acc: %f" %(self.step, dev_acc_mat, dev_acc_mismat, dev_acc_snli, test_acc_snli, mtrain_acc))
                        logger.Log("Step: %i\t Dev-matched cost: %f\t Dev-mismatched cost: %f\t Dev-SNLI cost: %f\t test-SNLI cost: %f\t MultiNLI train cost: %f" %(self.step, dev_cost_mat, dev_cost_mismat, dev_cost_snli, test_cost_snli, mtrain_cost))
                ...
                if self.best_dev_mat > 0.892 and config.training_completely_on_snli:
                    self.eval_step = 500
                    self.save_step = 500
                
                if self.best_dev_mat > 0.898 and config.training_completely_on_snli:
                    self.eval_step = 100
                    self.save_step = 100
                    self.dont_print_unnecessary_info = True 
    ...
    def classify(self, examples):
        ...
        for i in tqdm(range(total_batch + 1)):
            if i != total_batch:
                ...
                batch_len = len(examples)-self.batch_size*i
...
if config.preprocess_data_only:
    ...
    classifier.train(training_mnli[0:5000], training_snli, dev_matched, dev_mismatched, dev_snli,test_snli,image_feature)
